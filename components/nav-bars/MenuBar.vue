<template>
  <div
    class="top-0 right-0 flex flex-col items-center justify-start h-screen gap-4 px-6 py-10 bg-gray-100 xl:w-[15%] transition-base shadow"
  >
    <div
    ...
    >
      <div v-if="$store.getters.getProject == null">
        <h1 class="text-3xl font-bold text-center">Colmena Project</h1>
      </div>
      <div
        v-else
        class="flex flex-col items-center justify-center w-full text-gray-700 max-h-[30%] h-[30%] gap-6"
    ...
    <div
      class="flex flex-col max-h-[70%] h-[70%] w-full gap-2 text-lg font-semibold text-gray-500"
    >
      <nuxt-link
        v-for="(menuItem, index) in menu"
        :key="index"
        :to="localePath(menuItem.route)"
        class="flex items-center gap-4 px-4 py-2 transition-base hover:bg-colmenablue-600 rounded-xl hover:text-white"
