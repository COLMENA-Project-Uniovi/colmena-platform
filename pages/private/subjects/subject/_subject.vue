<template>
  <div class="grid grid-cols-2 gap-8 px-10 py-8">
    ...
    <div class="relative flex flex-col w-full gap-2 p-10 bg-white shadow-sm rounded-xl">
    ...
    this.subject.sessions.forEach((session) => {
      session.session_schedules.forEach((session_schedule) => {
        if (groups_id.includes(session_schedule.practice_group_id)) {
          ...
          session_schedule.practice_group.supervisor.username = this.getSupervisor(
            session_schedule.practice_group.supervisor.username
          );
          this.$store.commit(
            "setSupervisor",
            JSON.stringify(session_schedule.practice_group.supervisor)
          );
          const event = {
            highlight: {
              color: "purple",
