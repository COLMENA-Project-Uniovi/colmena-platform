<template>
  <div class="relative grid grid-cols-2 gap-8">
    ...
    <div
      v-if="families !== undefined"
      class="flex flex-col col-start-1 col-end-3 gap-4"
    >
      <h2 class="pl-4 text-2xl font-bold">Las familias de errores</h2>
      <ul class="grid w-full grid-cols-3 gap-4">
        <li v-for="family in Object.values(families)" :key="family.id">
          <div
            class="flex flex-col items-start justify-start h-full gap-1 p-5 text-white shadow-lg bg-gradient-to-tr from-colmenablue-600 via-colmenablue-600 to-colmenablue-400 rounded-xl transition-base"
          >
            <p class="text-xl font-semibold">{{ family.name }}</p>
            <p class="">{{ family.description }}</p>
          </div>
        </li>
      </ul>
    </div>
    ...
    >
      <div class="flex items-center justify-between w-full">
        <h2 class="pl-4 text-2xl font-bold">Los errores</h2>
        <form
          class="flex items-center justify-center gap-2 px-4 py-2 font-semibold bg-white shadow transition-base bg-gradient-to-tr rounded-xl h-fit hover:shadow-sm"
        >
      ...
      <ul class="grid w-full grid-cols-3 gap-4">
        <li v-for="error in errors" :key="error.id">
          <nuxt-link
            :to="localePath(`/private-area/errors/${error.id}`)"
            class="flex flex-col items-center justify-center gap-2 p-4 bg-white shadow rounded-xl transition-base hover:scale-105"
          >
            <p class="text-sm font-semibold">{{ error.name }}</p>
          </nuxt-link>
        </li>
      </ul>
    </div>
